...
void CompatSettings::configureDiagnostics(DiagnosticEngine& diagEngine) const {
    ...
    if (mode == CompatMode::Strict) {
        // IEEE 1800-2023 strict mode: maximum LRM compliance. The diagnostics
        // above are already promoted by the non-compat branch; this one is
        // only an error under strict mode.
        diagEngine.setBaselineSeverity(diag::MissingTimeScale, DiagnosticSeverity::Error);
    }
}
