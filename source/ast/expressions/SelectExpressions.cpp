...
Expression& MemberAccessExpression::fromSelector(
    ...
    if (expr.kind == ExpressionKind::StringLiteral) {
        ...
        auto& stringType = comp.getStringType();
        if (auto result = CallExpression::fromBuiltInMethod(comp, stringType.kind, expr,
                                                           selector.name, invocation, withClause, context)) {
            return *result;
        }
    ...
    switch (member->kind) {
        ...
        case SymbolKind::ConstraintBlock: {
            ...
            // sukimasim keeps this an error (upstream downgraded the general
            // dynamic-access case to warnIfNotProcedural).
            if (context.flags.has(ASTFlags::NonProcedural)) {
                context.addDiag(diag::DynamicNotProcedural, range);
                return badExpr(comp, &expr);
            }
            ...
            auto& constraintAccess = *comp.emplace<MemberAccessExpression>(
                comp.getVoidType(), expr, *member, range);
