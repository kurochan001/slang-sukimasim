...
const Symbol* PackageSymbol::findForImport(std::string_view lookupName) const {
    ...
    for (auto import : wildcardData->wildcardImports) {
        ...
        if (!exportsPackage && exportData) {
            for (auto decl : exportData->decls) {
                if (decl->item.kind == TokenKind::Star &&
                    decl->package.valueText() == package->name) {
                    exportsPackage = true;
                    break;
                }
            }
...
const PackageSymbol* GenericPackageDefSymbol::getSpecializationImpl(
    ...
    if (!exportDecls.empty()) {
        pkgSymbol->exportData = comp.emplace<PackageSymbol::ExportData>();
        pkgSymbol->exportData->decls = exportDecls.copy(comp);

        if (hasExplicitExports)
            pkgSymbol->exportData->explicitExportSyms = comp.allocSymbolMap();

        if (hasStarExports)
            pkgSymbol->exportData->wildcardExportPackages = comp.allocSymbolMap();
    }
