    ...
    auto copyStateInto = [&](Compilation& c, bool isFinal) {
        ...
        for (auto& entry : binds) {
            if (dbg_bind) {
                ...
                std::cerr << "[BIND_RESOLVE] definitionTarget: "
            ...
            if (entry.definitionTarget) {
                if (!entry.path.empty()) {
                    ...
                    if (dbg_bind) {
                        std::cerr << "[BIND_RESOLVE] Added bind to override node (instance path "
                                     "bind)\n";
                    }
                }
                else {
                    ...
                    if (def) {
                        ...
                        if (dbg_bind) {
                            std::cerr << "[BIND_RESOLVE] Added bind to definition: " << def->name
                                      << " (type bind)\n";
                        }
                    }
                }
